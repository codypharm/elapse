...
describe("FR-EXM-102 npm start", () => {
  it("creates the Product and names the runner, without opening a checkout session", async () => {
    ...
    expect(api.requests[1]?.body).toEqual({ name: "Serverless runtime", rate_usd_per_second: "0.002" });
  ...
  it("reuses an existing Product by name instead of creating another", async () => {
    const { api, out } = await run([{ id: "prod_old", name: "Serverless runtime", rate_usd_per_second: "0.002" }]);
