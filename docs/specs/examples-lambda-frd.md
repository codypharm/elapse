...
| Id | Requirement | Acceptance |
| --- | --- | --- |
| FR-EXM-120 | `POST /run?sub={sub_id}` with `{ input }` (the JSON request, no longer `{ code }`): if no active session → FR-EXM-114 (409 needs_start); else if the daily limit is reached → 429 (FR-EXM-140); else invoke the AWS runner and return `{ ok, result / error, ms, logs }`, where `result` carries the rendered image. The daily-limit and active checks run before any AWS call. | Tests: no session → 409; active + mock runner → 200 with the image payload; over limit → 429 with no runner call. |
| FR-EXM-121 | The executor is a **deep module**, one interface `run(input: RunInput): Promise<RunResult>`, two implementations: `awsRunner` (real, `@aws-sdk/client-lambda` `InvokeCommand` on `LAMBDA_FN`) and `mockRunner` (deterministic, no network, tests/CI). The real runner is default; the mock is a test/CI seam only (internal `LAMBDA_RUNNER_MODE=mock`), not a user option. | Unit tests on both against the same interface; `awsRunner` with the SDK client mocked. |
| FR-EXM-122 | The runner Lambda (`runner/index.mjs`, shipped) is a **real workload, not a code runner**: it receives `{ width, height, iterations }`, renders a Mandelbrot tile, and returns `{ ok, result: { png, width, height, iterations }, ms, logs }`. The PNG is encoded with **`node:zlib` only** — zero dependencies. Input is validated and **clamped** to a maximum width, height and iteration count so one request cannot exceed the function's time or response-payload budget. It **never executes caller-supplied code**. Compute time scales with the requested detail, which is what makes the per-second billing legible. | Committed runner returns a valid PNG for a known input, clamps an over-large request, and rejects a malformed one. |
...
| BR-EXM-108 | **Amended 2026-09-13 (stronger):** the example never executes caller-supplied code **anywhere** — not in the server process and not in the Lambda. The runner accepts structured input only and renders from it. This removes the arbitrary-code-execution risk that the AWS-only ADR and the README's Security section were written around. |
...
RunInput  = { width: number, height: number, iterations: number }   // clamped by the runner
RunResult = { ok: true, result: { png: string, width: number, height: number, iterations: number }, ms: number, logs: string[] }
