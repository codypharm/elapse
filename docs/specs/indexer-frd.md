...
| 2026-09-07 | Claude (for William) | FR-IDX-024's platform side is specified as worker FR-WRK-073: a stream the indexer never saw (older factory) is reconciled by the platform pulling its logs through ingest. The indexer-side re-POST of `failed` rows stays as written. |
