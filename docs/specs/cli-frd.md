...
| 2026-09-07 | Claude (for William) | Judge-pass dry run: `resolveBaseUrl` also reads `ELAPSE_API_URL` after `ELAPSE_BASE_URL`, so the Quickstart's single export serves curl, the SDK and the CLI. Help text and README updated; 29 tests green. **`@elapse/cli` is not yet on npm** — the Quickstart's `npx @elapse/cli listen` 404s until William publishes it (SDK is at 0.1.1). |
| 2026-09-11 | Claude (for William) | Found during the first live-mode run: the `listen` banner printed a hardcoded `Elapse CLI 0.1.0` while `CLI_VERSION` (read from `package.json`) was used correctly everywhere else, and `test/fixtures/listen-output.txt` pinned that literal — which `sync-snippets` copies verbatim into `cli-listen.mdx`, so the stale version was both asserted by a test and published on the docs site. Banner now reads `CLI_VERSION`; the test asserts the running version; fixture and snippet regenerated. Published as **0.1.3**. The fixture still carries a real version, so a future bump fails that test until it is regenerated — deliberate, so the docs snippet cannot drift again. |
