...
Every snippet on this page is extracted from the finished merchant in [`examples/saas`](https://github.com/elapsefinance/elapse/tree/master/examples/saas), so it runs as written. If you would rather read code than follow steps, clone that folder and run `npm start`.
...
<Steps>
  ...
  <Step title="Create a Checkout session and print its URL">
    <Tabs>
      <Tab title="TypeScript">
        <ExampleSession />
        Send the subscriber to `session.url`; it is the link you open in the last step. When they stop, Elapse returns them to `success_url` with `?session_id=cs_…` appended.
      </Tab>
      <Tab title="cURL">
        ```bash
  ...
  <Step title="Handle subscription.canceled">
    ...
    <Tabs>
      <Tab title="TypeScript">
        <ExampleVerify />
        <ExampleHandle />
        `constructEvent` throws on a missing, malformed, expired or mismatched signature. Anything it returns is genuine. The full handler, with the entitlement map and the six actions, is 45 lines in [`webhooks.ts`](https://github.com/elapsefinance/elapse/blob/master/examples/saas/src/webhooks.ts).
      </Tab>
      <Tab title="cURL">
        ```bash
  ...
  <Step title="Open the URL on your phone, Start, Stop">
    Open the Checkout URL from step 4 on a phone. Sign in with Face ID, press **Start**, watch the meter, press **Stop** after a few seconds. Your terminal shows `subscription.canceled` with `seconds_elapsed` and what was paid.
...
<CardGroup cols={2}>
  ...
  <Card title="Clone the finished merchant" icon="github" href="https://github.com/elapsefinance/elapse/tree/master/examples/saas">
