...
- **Test mode** escrows MockUSD. The checkout mints it for the subscriber, so a meter starts without anyone holding anything. It has the same shape as AUSD (six decimals, ERC-2612 permit), so a checkout that passes in test mode behaves the same in live mode. It is never deployed to mainnet.
