...
## Let subscribers manage their meter

Pause, resume and stop are the subscriber's actions, taken on the hosted meter page with their own signature. Your server cannot pause on their behalf, and there is no pause endpoint. Instead, link them there: every Subscription carries `manage_url`, the same page they checked out on. While the meter runs it shows the counter, **Stop**, and **Pause** if the Product allows it; after a stop it shows the receipt. Put the link where you would put "Manage subscription".
...
You find out what they did through webhooks: `subscription.updated` for a pause or resume, `subscription.canceled` for a stop. A stopped meter is final. To bill them again, create a new Checkout session.
