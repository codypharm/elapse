...
# Usage: ./deploy-testnet.sh <treasury-address> <keeper-address>
# The keeper is the relayer wallet; it is set in the deploy run itself.
set -euo pipefail
export PATH="$HOME/.foundry/bin:$PATH"
cd "$(dirname "$0")"
TREASURY="${1:?usage: ./deploy-testnet.sh <treasury-address> <keeper-address>}"
KEEPER="${2:?usage: ./deploy-testnet.sh <treasury-address> <keeper-address>}"
